...
def get_real_test_data(query_dir, gallery_dir, target_height, target_width, batch_size, workers):
    ...
    query_loader = DataLoader(
        Flip_Preprocessor(data_dir=query_dir, is_flip=True,
                     transform=test_transformer),
        batch_size=batch_size, num_workers=workers,
        shuffle=False,pin_memory=True)
    
    gallery_loader = DataLoader(
        Flip_Preprocessor(data_dir=gallery_dir, is_flip=True,
                     transform=test_transformer),
        batch_size=batch_size, num_workers=workers,
        shuffle=False,pin_memory=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resume', type=str, default='the_trained_model_path')
    ...
    # model
    parser.add_argument('--arch', type=str, default='cross_trihard_se_resnet152') 
    parser.add_argument('--num_classes', type=int, default=529) 
